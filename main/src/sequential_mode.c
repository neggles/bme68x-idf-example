    ...
    while (sample_count <= SAMPLE_COUNT) {
        ...
        for (uint8_t i = 0; i < n_fields; i++) {
            ...
            ESP_LOGI(TAG, "%u, %lu, %d, %lu, %lu, %lu, 0x%x, %d, %d", sample_count, (long unsigned int)time_ms,
                     (data[i].temperature / 100), (long unsigned int)data[i].pressure, (long unsigned int)(data[i].humidity / 1000),
                     (long unsigned int)data[i].gas_resistance, data[i].status, data[i].gas_index, data[i].meas_index);
#endif
            sample_count++;
        }
    }
