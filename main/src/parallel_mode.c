...
void bme68x_task(void *arg) {
    ...
    ESP_LOGI(TAG, "Print parallel mode data if mask for new data(0x80), gas measurement(0x20) and heater stability(0x10) are set");
    ...
    while (sample_count <= SAMPLE_COUNT) {
        ...
        for (uint8_t i = 0; i < n_fields; i++) {
            if (data[i].status == BME68X_VALID_DATA) {
                ...
                ESP_LOGI(TAG, "%u, %lu, %.2f, %.2f, %.2f, %.2f, 0x%x, %d, %d", sample_count, (long unsigned int)time_ms,
                         data[i].temperature, data[i].pressure, data[i].humidity, data[i].gas_resistance, data[i].status,
                         data[i].gas_index, data[i].meas_index);
#else
                ESP_LOGI(TAG, "%u, %lu, %d, %lu, %lu, %lu, 0x%x, %d, %d", sample_count, (long unsigned int)time_ms,
                         (data[i].temperature / 100), (long unsigned int)data[i].pressure,
                         (long unsigned int)(data[i].humidity / 1000), (long unsigned int)data[i].gas_resistance, data[i].status,
                         data[i].gas_index, data[i].meas_index);
#endif
                sample_count++;
            }
